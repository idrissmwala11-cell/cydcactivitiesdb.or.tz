        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900">{{ $skillsAttendance->date }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Teacher Name</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $skillsAttendance->teacher_name }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Lesson Topic</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $skillsAttendance->lesson_topic }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Present Count</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $skillsAttendance->present_count }}</p>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Absent Count</label>
                                <p class="mt-1 text-sm text-gray-900">{{ $skillsAttendance->absentParticipants->count() }}</p>
                            </div>
                            <div>
                    ...
                    <!-- Absent Participants -->
                    @if($skillsAttendance->absentParticipants->count() > 0)
                    <div class="mb-8">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Absent Participants ({{ $skillsAttendance->absentParticipants->count() }})</h3>
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <div class="space-x-3">
                            <a href="{{ route('skills-attendance.edit', $skillsAttendance) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                                Edit
                            </a>
                            <form action="{{ route('skills-attendance.destroy', $skillsAttendance) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this record?')">
                                @csrf
                                @method('DELETE')
