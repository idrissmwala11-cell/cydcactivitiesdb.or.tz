...
<head>
    ...
    <title>{{ $moduleTitle ?? 'Report' }} Print</title>
    <style>
        * {
    ...
    <div class="page-wrapper">
        ...
        <div class="summary-grid">
            <div class="summary-card">
                ...
                <div class="summary-value">{{ $centerId ?? 'N/A' }}</div>
            </div>

            <div class="summary-card">
                <span class="summary-label">Period</span>
                <div class="summary-value">{{ $periodLabels[$period ?? 'all'] ?? 'All Time' }}</div>
            </div>

            <div class="summary-card">
                <span class="summary-label">Total Records</span>
                <div class="summary-value">{{ isset($records) ? $records->count() : 0 }}</div>
            </div>
        </div>
                    ...
                    <div class="fields-grid">
                        @foreach($records as $index => $record)
                            <div class="field-item">
                                <span class="field-label">Record {{ $index + 1 }}</span>
                                <div class="field-value">
                                    <strong>Center ID:</strong> {{ strtoupper($record->center_id) }}<br>
                                    <strong>Total Users:</strong> {{ $record->total_users }}<br>
                                    <strong>First Registered:</strong> {{ \Carbon\Carbon::parse($record->first_registered_at)->format('d M Y') }}<br>
            ...
            @foreach($records as $index => $record)
                ...
                <div class="record-card">
                    ...
                    <div class="record-subheader">
                        ...
                        <div class="record-subheader-item">
                            <strong>Center ID:</strong> {{ strtoupper($record->user->center_id ?? $centerId ?? 'N/A') }}
                        </div>
                    </div>

                    @foreach($groupedFields as $sectionTitle => $fields)
                        <div class="section">
                            <div class="section-title {{ getSectionClass($sectionTitle) }}">
                                {{ $sectionTitle }}
                            </div>
                            ...
                            <div class="fields-grid">
                                @foreach($fields as $key => $value)
                                    <div class="field-item">
                                        <span class="field-label">{{ formatFieldLabel($key, $labelMap) }}</span>
